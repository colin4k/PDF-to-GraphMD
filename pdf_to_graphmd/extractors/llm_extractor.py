...
规则：
1. 尽可能多地提取有意义的实体和关系，构建丰富的知识网络
2. 使用描述性但简洁的实体ID
3. 确保关系中的所有实体ID都存在于实体列表中
4. 根据清晰度分配置信度分数（0.0-1.0）
5. 如果提到替代名称，请在别名中包含
6. **🚨 重要：所有中文内容必须使用简体中文，绝对禁止繁体字**
7. **🚨 重要：每个汉字都必须是简体形式（如：经济、时间、发展、应该等）**
8. **描述要使用现代简体中文的表达方式**
9. **积极寻找实体间的隐含关系，如因果关系、时间关系、从属关系等**
10. **即使关系不明显，也要尝试建立合理的连接以形成知识图谱网络**
11. **输出前请检查：确保没有出现經濟、設備、時間、應該、導致等繁体字**"""

            user_prompt = f"""从以下文本中提取知识图谱信息。请特别注意：
1. 提取所有提到的重要概念、人物、组织、地点等作为实体
2. 识别实体间的各种关系，包括明显的和隐含的关系  
3. 确保生成丰富的关系网络，让知识图谱具有良好的连通性
4. **🚨 关键：必须用标准简体中文输出所有内容**
5. **🚨 关键：禁止使用任何繁体字（如經濟、設備、時間等）**
6. **🚨 关键：输出前检查，确保所有汉字都是简体形式**
